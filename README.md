...
* **`utils.py` (As Ferramentas):** Contém a classe `SanitizadorMonetario`. Ela usa Expressões Regulares (`re`) para limpar os valores digitados pelo usuário (ex: converte "R$ 1.500,50" ou "1500.50" para um `float` seguro).
...
* Um token de bot do Telegram (Crie o seu conversando com o [@BotFather](https://t.me/botfather) no Telegram).

### 2. Passo a Passo

Clone este repositório:
```bash
git clone [https://github.com/seu-usuario/porquinBot-ProjetoDeSoftware.git](https://github.com/seu-usuario/porquinBot-ProjetoDeSoftware.git)
...
Inicie o bot através do `main.py`:
```bash
python main.py
```
...
* `/status_metas` - Acompanha a porcentagem de conclusão dos seus objetivos.
